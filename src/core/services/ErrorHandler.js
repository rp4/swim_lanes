...
export class ErrorHandler {
  ...
  recoverFromRenderError() {
    // Try to clear and re-render the diagram
    try {
      const event = new CustomEvent('diagram-recovery', {
        detail: { action: 're-render' },
      });
   ...
   */
  recoverFromDataError() {
    // Try to restore from localStorage backup
    try {
      const backup = localStorage.getItem('diagramBackup');
      if (backup) {
        const event = new CustomEvent('diagram-recovery', {
          detail: { action: 'restore', data: JSON.parse(backup) },
        });
        window.dispatchEvent(event);
      }
    } catch (e) {
